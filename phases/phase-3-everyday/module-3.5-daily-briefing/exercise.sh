...
python3 "$MCP_CALL" daily_note_add '{"content":"Solid day of work. Got the morning tasks done early and had time to plan ahead. The system is coming together and the crew is in a good rhythm.","note_type":"journal","mood":"focused"}' > "$TEMP_DIR/journal_result.json" 2>/dev/null

if [ $? -eq 0 ]; then
    echo -e "  \e[92m   OK — Journal entry added with mood: focused\e[0m"
else
    echo -e "  \e[91m   FAIL — Could not add journal entry\e[0m"
...
python3 "$MCP_CALL" daily_note_add '{"content":"Send the revised estimate to the Martinez project by Wednesday. Include the updated material costs and the 10 percent contingency.","note_type":"todo"}' > "$TEMP_DIR/todo_result.json" 2>/dev/null

if [ $? -eq 0 ]; then
    echo -e "  \e[92m   OK — Todo added\e[0m"
else
    echo -e "  \e[91m   FAIL — Could not add todo\e[0m"
    echo "     Check hints.md for troubleshooting."
fi
echo
...
python3 "$MCP_CALL" daily_note_add '{"content":"Truck inspection is due next month. Call the shop and schedule it before the end of the week so we do not get caught off guard.","note_type":"reminder"}' > "$TEMP_DIR/reminder_result.json" 2>/dev/null

if [ $? -eq 0 ]; then
    echo -e "  \e[92m   OK — Reminder added\e[0m"
else
    echo -e "  \e[91m   FAIL — Could not add reminder\e[0m"
    echo "     Check hints.md for troubleshooting."
fi
echo
...
python3 "$MCP_CALL" daily_briefing > "$TEMP_DIR/briefing_result.json" 2>/dev/null

if [ $? -eq 0 ]; then
    echo -e "  \e[92m   OK — Daily briefing generated\e[0m"
    echo
    echo "   --- YOUR BRIEFING ---"
    echo
    python3 -c "import json; d=json.load(open('$TEMP_DIR/briefing_result.json')); text=d.get('briefing',d.get('text',str(d))); print(text[:1000])" 2>/dev/null
    echo
    echo "   --- END BRIEFING ---"
else
    echo -e "  \e[91m   FAIL — Could not generate briefing\e[0m"
...
echo
echo "   Now run verify.sh to confirm everything stuck."
echo
