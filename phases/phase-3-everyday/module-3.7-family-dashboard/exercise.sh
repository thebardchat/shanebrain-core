...
if [ $? -eq 0 ]; then
    ...
    echo
    echo "   COLLECTIONS:"
    python3 -c "import json; d=json.load(open('$TEMP_DIR/health.json')); cols=d.get('collections',{}); [print(f'     {k:20s} {v:>5} objects') for k,v in cols.items()]; print(); print(f'     {\"TOTAL\":20s} {sum(cols.values()):>5} objects')" 2>/dev/null
    echo
    echo "   +--------------------------------------------------+"
else
...
python3 "$MCP_CALL" search_knowledge '{"query":"family"}' > "$TEMP_DIR/knowledge.json" 2>/dev/null
...
if [ $? -eq 0 ]; then
    ...
    echo '   +--------------------------------------------------+'
    echo
    python3 -c "import json; d=json.load(open('$TEMP_DIR/knowledge.json')); results=d.get('results',d.get('knowledge',[])); entries=results if isinstance(results,list) else [results]; [print(f'     [{e.get(\"category\",\"general\")}] {str(e.get(\"content\",e.get(\"text\",\"\")))[:100]}...') for e in entries[:5]]" 2>/dev/null
    echo
    echo "   +--------------------------------------------------+"
else
...
python3 "$MCP_CALL" get_top_friends > "$TEMP_DIR/friends.json" 2>/dev/null

if [ $? -eq 0 ]; then
    echo -e "  \e[92m   OK — Friend profiles retrieved\e[0m"
    echo
    echo "   +--------------------------------------------------+"
    echo "   |  RELATIONSHIP NETWORK                             |"
    echo "   +--------------------------------------------------+"
    echo
    python3 -c "import json; d=json.load(open('$TEMP_DIR/friends.json')); friends=d.get('friends',d.get('results',[])); entries=friends if isinstance(friends,list) else [friends]; [print(f'     {e.get(\"name\",\"Unknown\"):20s} strength: {e.get(\"relationship_strength\",e.get(\"strength\",\"?\"))}') for e in entries[:10] if isinstance(e,dict)]" 2>/dev/null
    echo
    echo "   +--------------------------------------------------+"
    echo
    ...
    echo -e "  \e[93m   WARNING — Could not retrieve friend profiles\e[0m"
...
python3 "$MCP_CALL" chat_with_shanebrain '{"message":"Give me a summary of what you know about me and my system. What data do you have, what can you help me with, and what should I focus on next?"}' > "$TEMP_DIR/chat.json" 2>/dev/null

if [ $? -eq 0 ]; then
    echo -e "  \e[92m   OK — ShaneBrain responded\e[0m"
    echo
    echo "   +--------------------------------------------------+"
    echo "   |  YOUR AI SAYS:                                    |"
    echo "   +--------------------------------------------------+"
    echo
    python3 -c "import json; d=json.load(open('$TEMP_DIR/chat.json')); text=d.get('response',d.get('text',d.get('message',str(d)))); print('    ',text[:800])" 2>/dev/null
    echo
    echo "   +--------------------------------------------------+"
else
    echo -e "  \e[91m   FAIL — Could not chat with ShaneBrain\e[0m"
...
echo
echo -e "     \e[92m+\e[0m Stores your documents in a private vault"
echo -e "     \e[92m+\e[0m Answers questions from your personal data"
echo -e "     \e[92m+\e[0m Writes drafts using your context"
echo -e "     \e[92m+\e[0m Locks down with security and privacy controls"
echo -e "     \e[92m+\e[0m Journals your days and generates briefings"
echo -e "     \e[92m+\e[0m Audits its own data footprint"
echo -e "     \e[92m+\e[0m Knows your people and your story"
echo -e "     \e[92m+\e[0m Talks to you about your life"
echo
...
echo
echo "   Now run verify.sh to lock in your completion."
echo
